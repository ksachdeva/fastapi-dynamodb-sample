...
_RECEIPE_TABLE_NAME = os.environ.get("RECIPE_TABLE_NAME")

if _RECEIPE_TABLE_NAME is None:
    raise ValueError("RECIPE_TABLE_NAME environment variable is not set")
...
# picks from the environment
session = boto3.Session()

dynamodb = session.resource("dynamodb")
table = dynamodb.Table(_RECEIPE_TABLE_NAME)
    ...
    try:
        table.put_item(
            Item={
                ...
                "steps": [steps.model_dump() for steps in recipe.steps],
            }
        )
